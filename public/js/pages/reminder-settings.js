...
import { showLoading } from "../modules/loading-indicator.js";
...
async function refreshPushDevice({ timeoutMs = 5_000 } = {}) {
...
function renderEmail(value) {
  ...
  const button = byId("verifyEmail"); const visible = ["OFF", "VERIFICATION_REQUIRED", "VERIFICATION_PENDING"].includes(value.state); setVisible("verifyEmail", visible);
  if (emailCountdown) clearInterval(emailCountdown); emailCountdown = null;
  if (visible) {
    const resend = value.hasPreviousRequest === true || value.state === "VERIFICATION_PENDING"; const deadline = Date.now() + Math.max(0, value.retryAfterSeconds || 0) * 1000;
    const update = () => { const remaining = Math.max(0, Math.ceil((deadline - Date.now()) / 1000)); button.disabled = remaining > 0; button.dataset.countdownDisabled = remaining > 0 ? "true" : "false"; button.textContent = remaining > 0 ? `Resend available in ${formatResendDelay(remaining)}` : resend ? "Resend verification email" : "Send verification email"; if (!remaining && emailCountdown) { clearInterval(emailCountdown); emailCountdown = null; } };
    update(); if (value.retryAfterSeconds > 0) emailCountdown = setInterval(update, 250);
  }
  setVisible("enableEmail", value.state === "USER_DISABLED"); setVisible("disableEmail", value.state === "ENABLED");
}
...
async function load() { showLoading(byId("pageStatus"), "Loading reminder options…"); try { const result = await loadChannelStatuses(); if (result.push.value) renderPush(result.push.value); else byId("pushStatus").textContent = "Temporarily unavailable"; if (result.email.value) renderEmail(result.email.value); else byId("emailStatus").textContent = "Temporarily unavailable"; if (result.calendar.value) renderCalendar(result.calendar.value); else byId("calendarStatus").textContent = "Temporarily unavailable"; byId("pageStatus").textContent = Object.values(result).some((item) => item.error) ? "Some reminder options could not be loaded. Other options remain available." : "Reminder options loaded."; if (result.push.value) void refreshPushDevice().catch(() => { byId("pushStatus").textContent = "Temporarily unavailable"; }); } catch (error) { if (error.message === "SESSION_EXPIRED") location.href = "/index.html?returnTo=%2Freminder-settings.html"; else byId("pageStatus").textContent = "Reminder options are temporarily unavailable. Return Home and try again."; } }
async function pending(button, action, afterLoad) { button.disabled = true; try { const result = await action(); await load(); if (afterLoad) afterLoad(result); } catch (error) { if (error.message === "SESSION_EXPIRED") location.href = "/index.html?returnTo=%2Freminder-settings.html"; else byId("pageStatus").textContent = "That change could not be completed. Try again."; } finally { if (button.dataset.countdownDisabled !== "true") button.disabled = false; button.focus(); } }
...
byId("disablePush").addEventListener("click", (event) => pending(event.currentTarget, async () => { if (!currentPushSubscription) return; await fetch("/api/user/reminders/push/subscription", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ endpoint: currentPushSubscription.endpoint }) }); await currentPushSubscription.unsubscribe(); currentPushSubscription = null; }));
byId("disableAllPush").addEventListener("click", (event) => pending(event.currentTarget, async () => { await fetch("/api/user/reminders/push/subscriptions", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: "{}" }); if (currentPushSubscription) await currentPushSubscription.unsubscribe(); currentPushSubscription = null; }));
byId("verifyEmail").addEventListener("click", (event) => pending(event.currentTarget, () => mutateEmpty("/api/user/reminders/email/verification-requests"), (result) => { byId("emailConfirmation").textContent = result.message || ""; }));
byId("enableEmail").addEventListener("click", (event) => pending(event.currentTarget, () => mutateEmpty("/api/user/reminders/email/enable")));
byId("disableEmail").addEventListener("click", (event) => pending(event.currentTarget, () => mutateEmpty("/api/user/reminders/email/disable")));
