...
export function buildBuybackView(buyback) {
  ...
  return {
    ...buyback,
    heading: buyback.status === "PENDING_USER_REQUEST" ? "Week 2 buyback request pending" : buyback.status === "UNAVAILABLE" ? "Week 2 buyback temporarily unavailable" : "Week 2 Track buyback",
...
export function renderBuyback(buyback, { fetchImpl = fetch, modalApi = window.bootstrap?.Modal } = {}) {
  ...
  form.replaceChildren(...view.tracks.map((track) => {
    ...
    const label = document.createElement("label"); label.className = "form-check-label"; label.htmlFor = input.id; label.textContent = `Track ${track.trackId} — Week 1 Pick: ${track.weekOnePick}${track.resolution ? ` (${track.resolution.toLowerCase()})` : ""}`;
  ...
  decline.onclick = () => {
    document.getElementById("buybackConfirmationSummary").textContent = "Continue without buying back any Track? This permanently closes the Week 2 offer and unlocks Picks for surviving Tracks.";
    document.getElementById("buybackConfirmation").hidden = false;
