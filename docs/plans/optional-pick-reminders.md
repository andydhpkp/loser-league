...
## PR 6 fixed contracts and status
...
- Public shell: `/manifest.webmanifest`, `/service-worker.js`, `/offline.html`, and local 192/512/maskable icons. The worker version-caches only the offline page, shared CSS, and icons. Navigation is network-first; APIs and authenticated responses are never intercepted. Updates wait for safe User action: the update-available event exposes **Reload now**, which activates the waiting worker and reloads only after the browser confirms the controller change.
...
- Hidden authenticated routes are `GET /api/user/reminders/email`, `POST /api/user/reminders/email/verification-requests`, `POST /api/user/reminders/email/enable`, and `POST /api/user/reminders/email/disable`. Mutation bodies must be exactly `{}`. They use only the session User, require effective Pick Reminders access, set `Cache-Control: private, no-store`, and never accept or return a User ID or full destination. Status returns `maskedDestination`, non-sensitive resend eligibility/timing, and one of `OFF`, `VERIFICATION_REQUIRED`, `VERIFICATION_PENDING`, `ENABLED`, `USER_DISABLED`, or `TEMPORARILY_UNAVAILABLE`.
...
- Verification requests lock the User and verification history. The settings page keeps a manual resend action visible, counts down automatically, and enables it without a reload. One potentially delivered request is allowed per ten minutes and five per rolling 24 hours, including boundary-safe concurrent requests; accepted and ambiguous results consume the limits, while definite failures do not. A limited response is HTTP 429 with a persisted `Retry-After` value and `{ state: "RATE_LIMITED", retryAfterSeconds }`; it reveals no counters. A potentially delivered replacement supersedes earlier unused links, while a definite failure preserves the earlier usable link. Cleanup deletes at most 100 expired/superseded request and expired opt-out rows per transaction and reports counts only.
...
## PR 5 fixed contracts: shared Pick-deadline calendar

- The compatibility route is public `GET /calendar/pick-deadlines.ics`. It is one sessionless, tokenless, cross-season feed. The hidden PR 6 discovery seam is authenticated `GET /api/user/reminders/calendar`; it returns availability, canonical HTTPS and `webcal:` links, `LINK_PROVIDED`, and the explicit fact that completion is not detectable. It is private/no-store and requires effective Pick Reminders access.
- `PUBLIC_APP_ORIGIN` is the sole URL authority. Event UIDs are `pick-deadline-<year>-<phase>-<round>@calendar.loser-league.app`; they contain no database or User identity and never change with a deadline. One `calendar_event` exists per year/phase/round. A meaningful published deadline or status change increments `SEQUENCE`; evidence-only and identical refreshes do not. Previously published invalid future events become `STATUS:CANCELLED` under the same UID.
- Events contain only `Loser League Picks Due`, UTC `DTSTART`, canonical dashboard `URL`, revision metadata, and a display `VALARM` with `TRIGGER:-PT24H`. Serialization is deterministic RFC 5545 with CRLF, escaping, and 75-octet folding. No library dependency is added.
- A dedicated season-schedule loader reuses the existing Fixture/ESPN shapes and authoritative earliest-kickoff validation. Preseason uses ESPN weeks 1–4; regular/playoff schedule evidence comes from one Fixture Download season response and maps rounds 1–18 to `REGULAR` and 19–22 to `PLAYOFF`. Week 0 and invalid rounds are omitted independently. Tests inject fake responses.
- `calendar_event` stores stable identity, deadline, status, sequence, source hash, and publication/cancellation times. Singleton `calendar_feed_state` stores only the last trustworthy exact representation, SHA-256 ETag evidence, Last-Modified, refresh time, and version. Refresh fetches/validates before the transaction, then locks publication state and events while reconciling. Startup plus a 15-minute recovery interval owns refresh; cleanup runs daily in batches of 100.
- The feed uses `text/calendar; charset=utf-8`, `Cache-Control: public, max-age=300`, an exact-representation strong `ETag`, and durable `Last-Modified`. `If-None-Match` takes precedence over `If-Modified-Since`. Provider failure does not mutate state, ETag, sequence, or Last-Modified; the last trustworthy representation remains served. Before the first trustworthy refresh, a deterministic valid empty calendar is served.
- `PICK_REMINDERS_CALENDAR_AVAILABLE` is strict and fail-closed. It is independent from email/push delivery controls and the outbound reminder system. When off or unconfigured, the public route returns the fixed empty calendar without deleting durable state. The master control still governs authenticated in-app discovery only.
- Confirmed events remain through 30 days after their deadline. Cancellation evidence remains for 30 days after cancellation. Old-season events within those windows coexist deterministically with the active season; the URL never changes. Cleanup never deletes `calendar_feed_state`.
- Client content covers Apple Calendar, Google Calendar, and Outlook subscription/removal. Clients control polling and alerts, may change or ignore the suggested alarm, and may alert after Picks are complete. Loser League never reports a subscription as enabled and cannot detect or remove it. The final settings/Help UI remains PR 6 scope.
