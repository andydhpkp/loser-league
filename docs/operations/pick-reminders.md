...
SMTP uses authenticated TLS to `smtp.gmail.com:465`. The sender inbox and replies must be monitored by an organizer. Gmail acceptance means only that Gmail accepted the SMTP transaction; it does not prove inbox delivery or reading. Consumer Gmail currently documents a 500-message/day limit, and sending may pause for 1–24 hours after a limit. Potentially delivered verification requests are independently limited to one per User per ten minutes and five per rolling 24 hours. The settings page exposes only a manual resend countdown. Accepted and ambiguous sends consume the limits because an ambiguous handoff may have delivered; definite failures do not and preserve an earlier usable verification link.
