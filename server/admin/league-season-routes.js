  ...
  router.get("/", async (_req, res, next) => {
    try {
      const [openSeason, unassignedTrackCount] = await Promise.all([
        LeagueSeason.findOne({ where: { open_slot: 1 }, attributes: ["id", "year", "state", "current_week", "state_version"] }),
      ...
      res.json({
        leagueSeason: season ? { id: season.id, year: season.year, state: season.state, week: season.current_week, stateVersion: season.state_version } : null,
