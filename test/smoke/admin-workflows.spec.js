...
const users = [
  { id: 3, first_name: "Alice", last_name: "Able", username: "alice", user_record: [], tracks: [] },
  { id: 4, first_name: "Bob", last_name: "Baker", username: "bob", user_record: [], tracks: [] },
];
...
for (const [workflow, title, expectedAction] of [
  ...
  ["View Statistics", "View Statistics", "Active Tracks"],
]) {
...
test("workflow back navigation uses the established outlined button treatment", async ({ page }) => {
  await page.getByRole("button", { name: "View Statistics" }).click();
  const back = page.getByRole("button", { name: "Back to Admin Home" });
  await expect(back).toHaveClass(/btn-outline-primary/);
